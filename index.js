...
const tempStack = fs.readFileSync(
  `${__dirname}/templates/template-stack.html`,
  'utf-8'
);
const tempGuitar = fs.readFileSync(
  `${__dirname}/templates/template-guitar.html`,
  'utf-8'
);

const slugs = dataObj.map((item) =>
  slugify(`${item.manufacturer} ${item.name}`, { lower: true })
);
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/' || pathname === '/overview') {
    const guitarHTML = dataObj
      .map((item) => replaceTemplate(tempStack, item))
      .join('');
